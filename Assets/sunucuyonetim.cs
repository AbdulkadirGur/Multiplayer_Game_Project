...
public class sunucuyonetim : MonoBehaviourPunCallbacks
{
    /* Bilinmesi gerekenler
        PhotonNetwork.ConnectUsingSettings(); // servere baglanma istegi

        PhotonNetwork.JoinLobby(); // lobiye baglan

        PhotonNetwork.JoinRoom("oda ismi"); // odaya baglan

        PhotonNetwork.JoinRandomRoom();  // rastgele bir odaya baglanir

        PhotonNetwork.CreateRoom("oda ismi ", oda_ayarlari );//oda olusturmaya yarar

        PhotonNetwork.JoinOrCreateRoom("oda ismi ", oda_ayarlari, TypedLobby.Default); // gir veya oda olustur

        PhotonNetwork.LeaveLobby(); //Lobiden ayril
        PhotonNetwork.LeaveRoom(); //odaden ayril
        */
    ...
    public override void OnPlayerLeftRoom(Player otherPlayer) // bir kullanici ciktiginda calisir....
    {
